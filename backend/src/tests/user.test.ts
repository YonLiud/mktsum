...
describe('userService', () => {
  ...
  it('getAllWithTickers — returns all users with their tickers', async () => {
    const u1 = await userService.create({ name: 'Test_GetAllWithTickers1', ntfy_topic: 'topic1' })
    const u2 = await userService.create({ name: 'Test_GetAllWithTickers2', ntfy_topic: 'topic2' })
    createdIds.push(u1.user_id, u2.user_id)
    const { watchlistService } = await import('../services/watchlistService.ts')
    await watchlistService.addTicker(u1.user_id, 'AAPL')
    await watchlistService.addTicker(u1.user_id, 'MSFT')
    await watchlistService.addTicker(u2.user_id, 'GOOGL')
    const allWithTickers = await userService.getAllWithTickers()
    expect(Array.isArray(allWithTickers)).toBe(true)
    const u1Found = allWithTickers.find((u: any) => u.user_id === u1.user_id)
    const u2Found = allWithTickers.find((u: any) => u.user_id === u2.user_id)
    expect(u1Found?.watchlist.length).toBe(2)
    expect(u1Found?.watchlist.map((t: any) => t.ticker)).toContain('AAPL')
    expect(u1Found?.watchlist.map((t: any) => t.ticker)).toContain('MSFT')
    expect(u2Found?.watchlist.length).toBe(1)
    expect(u2Found?.watchlist.map((t: any) => t.ticker)).toContain('GOOGL')
  })
})
...
describe('userController (HTTP)', () => {
  ...
  it('GET /with-tickers — returns all users with their tickers', async () => {
    const u1 = await userService.create({ name: 'Test_HTTP_AllWithTickers1', ntfy_topic: 'tickers1' })
    const u2 = await userService.create({ name: 'Test_HTTP_AllWithTickers2', ntfy_topic: 'tickers2' })
    createdIds.push(u1.user_id, u2.user_id)
    const { watchlistService } = await import('../services/watchlistService.ts')
    await watchlistService.addTicker(u1.user_id, 'TSLA')
    await watchlistService.addTicker(u2.user_id, 'AMZN')
    const res = await request(app).get('/with-tickers')
    expect(res.status).toBe(200)
    expect(Array.isArray(res.body)).toBe(true)
    const u1Found = res.body.find((u: any) => u.user_id === u1.user_id)
    const u2Found = res.body.find((u: any) => u.user_id === u2.user_id)
    expect(u1Found?.watchlist).toBeTruthy()
    expect(u1Found?.watchlist.some((t: any) => t.ticker === 'TSLA')).toBe(true)
    expect(u2Found?.watchlist.some((t: any) => t.ticker === 'AMZN')).toBe(true)
  })
})
